...
void LuxCoreEngineMaterial::UpdateLuxSDL()
{
  scene->SceneLux()->Parse(
	    luxrays::Property("scene.materials." + Name() + ".type")(this->materialType) <<
	    luxrays::Property("scene.materials." + Name() + ".kd")(this->diffuseR, this->diffuseG, this->diffuseB) <<
      luxrays::Property("scene.materials." + Name() + ".emission")(this->emissiveR, this->emissiveG, this->emissiveB));
}
