...
const std::string RESOURCE_PATH = "ign_rendering/examples/egl_demo/media";
...
int main(int _argc, char** _argv)
{
  ...
  std::map<std::string, std::string> param_map = {{"headless", "1"}};
  RenderEngine *engine = rendering::engine("ogre2", param_map);
  ...
  ignition::rendering::Image cameraImage = camera->CreateImage();
  camera->Capture(cameraImage);

  auto formatStr =
      ignition::rendering::PixelUtil::Name(camera->ImageFormat());
  ...
  std::string filename = ignition::common::joinPaths(ignition::common::cwd(), "test.png");
