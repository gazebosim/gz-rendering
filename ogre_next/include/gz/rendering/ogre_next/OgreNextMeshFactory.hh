 ...
 *
 */
#ifndef GZ_RENDERING_OGRE_NEXT_OGRE_NEXTMESHFACTORY_HH_
#define GZ_RENDERING_OGRE_NEXT_OGRE_NEXTMESHFACTORY_HH_
...
namespace gz
{
  namespace rendering
  {
    ...
    //
    // forward declaration
    class OgreNextMeshFactoryPrivate;
    class OgreNextSubMeshStoreFactoryPrivate;

    /// \brief OgreNext.x implementation of the mesh factory class
    class GZ_RENDERING_OGRE_NEXT_VISIBLE OgreNextMeshFactory
    {
      /// \brief Constructor
      /// \param[in] _scene Pointer to the scene
      public: explicit OgreNextMeshFactory(OgreNextScenePtr _scene);

      /// \brief Destructor
      public: virtual ~OgreNextMeshFactory();

      /// \brief Create a mesh from a descriptor
      /// \param[in] _desc Mesh descriptor containing data needed to create a
      /// mesh
      public: virtual OgreNextMeshPtr Create(const MeshDescriptor &_desc);
      ...
      private: std::unique_ptr<OgreNextMeshFactoryPrivate> dataPtr;
    };

    /// \brief OgreNext.x implementation of a submesh store factory class
    class GZ_RENDERING_OGRE_NEXT_VISIBLE OgreNextSubMeshStoreFactory
    {
      /// \brief Constructor
      /// \param[in] _scene Pointer to the scene object
      /// \param[in] _item Parent ogre item
      public: OgreNextSubMeshStoreFactory(OgreNextScenePtr _scene,
                  Ogre::Item *_item);

      /// \brief Destructor
      public: virtual ~OgreNextSubMeshStoreFactory();

      /// \brief Create the submeshes
      /// \return A store containing all the submeshes
      public: virtual OgreNextSubMeshStorePtr Create();
      ...
      protected: virtual OgreNextSubMeshPtr CreateSubMesh(unsigned int _index);
      ...
      /// \brief Pointer to private data class
      private: std::unique_ptr<OgreNextSubMeshStoreFactoryPrivate> dataPtr;
    };
    }
  }
}
