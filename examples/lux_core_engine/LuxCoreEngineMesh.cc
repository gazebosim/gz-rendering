...
std::vector<float> inverseMatrix(float* m)
{
  ...
  float det = m[0] * result[0] + m[1] * result[4] + m[2] * result[8] + m[3] * result[12];
  det = 1.0 / det;
...
void LuxCoreEngineMesh::UpdateTransformation()
{
  if (SubMeshes()->Size() > 0)
  {
    ...
    {
      std::vector<float> result = multiplyMatrix(this->rotationMatrix, this->scaleMatrix);
      ...
      std::vector<float> offsetMatrix = multiplyMatrix(inversePrevious.data(), result.data());
      scene->SceneLux()->UpdateObjectTransformation(SubMeshes()->GetByIndex(i)->Name(), offsetMatrix.data());
