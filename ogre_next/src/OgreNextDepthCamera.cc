...
#include "gz/rendering/ogre_next/OgreNextGaussianNoisePass.hh"
...
#include "OgreNextParticleNoiseListener.hh"
...
/// The class implementation is very similar to OgreNextGaussianNoisePass but
/// uses a different shader material for apply noise to depth cameras
/// This class is added here since we can not modify OgreNextGaussianNoisePass
/// as it would break ABI
class OgreNextDepthGaussianNoisePass : public OgreNextGaussianNoisePass
{
  /// \brief Constructor
  public: OgreNextDepthGaussianNoisePass() {}

  /// \brief Destructor
  public: virtual ~OgreNextDepthGaussianNoisePass() {}
...
/// \internal
/// \brief Private data for the OgreNextDepthCamera class
class gz::rendering::OgreNextDepthCameraPrivate
{
  ...
  /// emitter region
  public: std::unique_ptr<OgreNextParticleNoiseListener> particleNoiseListener;
...
//////////////////////////////////////////////////
void OgreNextDepthGaussianNoisePass::PreRender(const CameraPtr &/*_camera*/)
{
  GZ_PROFILE("Ogre2DepthGaussianNoisePass::PreRender");
  // This function is similar to OgreNextGaussianNoisePass but duplicated here
  // for OgreNextDepthCamera
...
//////////////////////////////////////////////////
void OgreNextDepthGaussianNoisePass::CreateRenderPass()
{
  // This function is similar to OgreNextGaussianNoisePass but duplicated here
  // for OgreNextDepthCamera.
...
//////////////////////////////////////////////////
OgreNextDepthCamera::OgreNextDepthCamera()
  : dataPtr(new OgreNextDepthCameraPrivate())
{
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::Destroy()
{
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::CreateCamera()
{
...
/////////////////////////////////////////////////
void OgreNextDepthCamera::CreateRenderTexture()
{
...
/////////////////////////////////////////////////////////
void OgreNextDepthCamera::CreateDepthTexture()
{
  ...
  if (!ogreCompMgr->hasWorkspaceDefinition(wsDefName))
  {
      ...
      // scene pass - opaque
      {
        ...
        else
        {
          ...
          passScene->setAllClearColours(
              OgreNextConversions::Convert(this->Scene()->BackgroundColor()));
    ...
    depthTargetDef->setNumPasses(1);
    {
      ...
      passScene->setVisibilityMask(
        GZ_VISIBILITY_ALL & ~OgreNextParticleEmitter::kParticleVisibilityFlags);
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::CreateWorkspaceInstance()
{
  auto engine = OgreNextRenderEngine::Instance();
  auto ogreRoot = engine->OgreRoot();
  Ogre::CompositorManager2 *ogreCompMgr = ogreRoot->getCompositorManager2();
  ...
  for (const auto &c : channelsTex)
  {
    if (c->getPixelFormat() == Ogre::PFG_R8_UNORM)
    {
      ...
      this->dataPtr->particleNoiseListener.reset(
          new OgreNextParticleNoiseListener(this->scene,
...
/////////////////////////////////////////////////
math::Matrix4d OgreNextDepthCamera::ProjectionMatrix() const
{
  return OgreNextConversions::Convert(this->ogreCamera->getProjectionMatrix());
}

/////////////////////////////////////////////////
void OgreNextDepthCamera::SetProjectionMatrix(const math::Matrix4d &_matrix)
{
  BaseDepthCamera::SetProjectionMatrix(_matrix);
  this->ogreCamera->setCustomProjectionMatrix(
      true, OgreNextConversions::Convert(this->projectionMatrix));
}

//////////////////////////////////////////////////
void OgreNextDepthCamera::Render()
{
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::PreRender()
{
  ...
  if (this->dataPtr->colorTargetDef)
  {
    ...
    GZ_ASSERT(colorPasses.size() > 2u,
        "OgreNextDepthCamera color target should contain more than 2 passes");
    GZ_ASSERT(colorPasses[0]->getType() == Ogre::PASS_CLEAR,
        "OgreNextDepthCamera color target should start with a clear pass");
  ...
  if (this->dataPtr->particleTargetDef)
  {
    ...
    GZ_ASSERT(particlePasses.size() == 2u,
        "OgreNextDepthCamera particle target should 2 passes");
    GZ_ASSERT(particlePasses[0]->getType() == Ogre::PASS_CLEAR,
        "OgreNextDepthCamera particle target should start with a clear pass");
    GZ_ASSERT(particlePasses[1]->getType() == Ogre::PASS_SCENE,
        "OgreNextDepthCamera particle target should end with a scene pass");
  ...
  OgreNextRenderTarget::UpdateRenderPassChain(
      this->dataPtr->ogreCompositorWorkspace,
      this->dataPtr->ogreCompositorWorkspaceDef,
      this->dataPtr->ogreCompositorBaseNodeDef,
      this->dataPtr->ogreCompositorFinalNodeDef,
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::PostRender()
{
...
//////////////////////////////////////////////////
const float *OgreNextDepthCamera::DepthData() const
{
  return this->dataPtr->depthBuffer;
}

//////////////////////////////////////////////////
common::ConnectionPtr OgreNextDepthCamera::ConnectNewDepthFrame(
...
//////////////////////////////////////////////////
common::ConnectionPtr OgreNextDepthCamera::ConnectNewRgbPointCloud(
    std::function<void(const float *, unsigned int, unsigned int,
      unsigned int, const std::string &)>  _subscriber)
{
...
//////////////////////////////////////////////////
RenderTargetPtr OgreNextDepthCamera::RenderTarget() const
{
  return this->dataPtr->depthTexture;
}

//////////////////////////////////////////////////
double OgreNextDepthCamera::LimitFOV(const double _fov)
{
  return std::min(std::max(0.001, _fov), GZ_PI * 0.999);
}

//////////////////////////////////////////////////
void OgreNextDepthCamera::SetNearClipPlane(const double _near)
{
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::SetFarClipPlane(const double _far)
{
...
//////////////////////////////////////////////////
double OgreNextDepthCamera::NearClipPlane() const
{
  return BaseDepthCamera::NearClipPlane();
}

//////////////////////////////////////////////////
double OgreNextDepthCamera::FarClipPlane() const
{
  return BaseDepthCamera::FarClipPlane();
}

//////////////////////////////////////////////////
void OgreNextDepthCamera::SetShadowsDirty()
{
  this->SetShadowsNodeDefDirty();
}

//////////////////////////////////////////////////
void OgreNextDepthCamera::SetShadowsNodeDefDirty()
{
  if (!this->dataPtr->ogreCompositorWorkspace)
    return;
...
//////////////////////////////////////////////////
void OgreNextDepthCamera::AddRenderPass(const RenderPassPtr &_pass)
{
  ...
  // class or extend the OgreNextGaussianNoisePass to handle both color and
  // depth cameras
  std::shared_ptr<OgreNextGaussianNoisePass> pass =
      std::dynamic_pointer_cast<OgreNextGaussianNoisePass>(_pass);
  if (!pass)
  {
  ...
  std::shared_ptr<OgreNextDepthGaussianNoisePass> depthNoisePass =
    std::make_shared<OgreNextDepthGaussianNoisePass>();
...
//////////////////////////////////////////////////
Ogre::Camera *OgreNextDepthCamera::OgreCamera() const
{
