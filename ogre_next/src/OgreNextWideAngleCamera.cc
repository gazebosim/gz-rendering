...
/// via OgreNextWideAngleCamera.
class GZ_RENDERING_OGRE_NEXT_HIDDEN
    OgreNextWideAngleCameraWorkspaceListenerPrivate
      : public Ogre::CompositorWorkspaceListener
{
  public: gz::rendering::OgreNextWideAngleCamera &owner;

  /// \brief constructor
  /// \param[in] _scene the scene manager responsible for rendering
  public: explicit OgreNextWideAngleCameraWorkspaceListenerPrivate(
        gz::rendering::OgreNextWideAngleCamera &_owner) :
    owner(_owner)
  {
  }
...
class gz::rendering::OgreNextWideAngleCamera::Implementation
{
  // clang-format off
  ...
  /// \brief Dummy texture
  public: OgreNextRenderTexturePtr wideAngleTexture;
  ...
  /// \brief See OgreNextWideAngleCameraWorkspaceListenerPrivate
  public: OgreNextWideAngleCameraWorkspaceListenerPrivate workspaceListener;
  ...
  explicit Implementation(gz::rendering::OgreNextWideAngleCamera &_owner) :
...
//////////////////////////////////////////////////
OgreNextWideAngleCamera::OgreNextWideAngleCamera() :
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::Init()
{
...
/////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateRenderTexture()
{
  this->DestroyRenderTexture();
  RenderTexturePtr base = this->scene->CreateRenderTexture();
  this->dataPtr->wideAngleTexture =
    std::dynamic_pointer_cast<OgreNextRenderTexture>(base);
  this->dataPtr->wideAngleTexture->SetWidth(1);
  this->dataPtr->wideAngleTexture->SetHeight(1);
}

//////////////////////////////////////////////////
void OgreNextWideAngleCamera::DestroyRenderTexture()
{
  if (this->dataPtr->wideAngleTexture)
  {
    dynamic_cast<OgreNextRenderTexture *>(this->dataPtr->wideAngleTexture.get())
      ->Destroy();
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::PreRender()
{
  ...
  else
  {
    if (!this->dataPtr->ogreCompositorWorkspace[0])
    {
      ...
      const uint8_t msaa = OgreNextRenderTarget::TargetFSAA(
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::Destroy()
{
  this->RemoveAllRenderPasses();
  this->DestroyTextures();
  this->DestroyRenderTexture();
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::DestroyTextures()
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::AddRenderPass(const RenderPassPtr &_pass)
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::RemoveRenderPass(const RenderPassPtr &_pass)
{
  // Do NOT pass it to super class.
  ...
  else
  {
    ...
    {
      gzwarn << "OgreNextWideAngleCamera::RemoveRenderPass pass not found. "
                "This is fine if you called this function twice. But it may "
                "not be fine if you changed the value "
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::RemoveAllRenderPasses()
{
  for (RenderPassPtr &pass : this->dataPtr->renderPasses)
  {
    pass->Destroy();
...
//////////////////////////////////////////////////
uint32_t OgreNextWideAngleCamera::EnvTextureSize() const
{
  return this->dataPtr->envTextureSize;
}

//////////////////////////////////////////////////
void OgreNextWideAngleCamera::SetEnvTextureSize(uint32_t _size)
{
  this->dataPtr->envTextureSize = _size;
}

/////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateCamera()
{
...
//////////////////////////////////////////////////
std::string OgreNextWideAngleCamera::WorkspaceFinalPassDefinitionName() const
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateStitchWorkspaceDefinition()
{
  ...
  for (RenderPassPtr &pass : this->dataPtr->finalStitchRenderPasses)
  {
    ...
    if (ogre_nextRenderPass->IsEnabled())
    {
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateStitchWorkspace()
{
  ...
  for (RenderPassPtr &pass : this->dataPtr->finalStitchRenderPasses)
  {
    OgreNextRenderPass *ogre_nextRenderPass =
      dynamic_cast<OgreNextRenderPass *>(pass.get());
    ogre_nextRenderPass->WorkspaceAdded(this->dataPtr->ogreCompositorFinalPass);
  }
}

//////////////////////////////////////////////////
void OgreNextWideAngleCamera::DestroyStitchWorkspace()
{
  ...
  if (this->dataPtr->ogreCompositorFinalPass)
  {
    ...
    for (RenderPassPtr &pass : this->dataPtr->finalStitchRenderPasses)
    {
      OgreNextRenderPass *ogre_nextRenderPass =
        dynamic_cast<OgreNextRenderPass *>(pass.get());
      ogre_nextRenderPass->WorkspaceRemoved(
          this->dataPtr->ogreCompositorFinalPass);
    }
...
//////////////////////////////////////////////////
bool OgreNextWideAngleCamera::HasTempStitchTexture() const
{
  return this->dataPtr->ogreStitchTexture[kStichTmpTexture];
}

//////////////////////////////////////////////////
bool OgreNextWideAngleCamera::NeedsTempStitchTexture() const
{
  return !this->dataPtr->finalStitchRenderPasses.empty();
}

//////////////////////////////////////////////////
uint32_t OgreNextWideAngleCamera::TempStitchTextureChannel() const
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::SetupMSAA(Ogre::CompositorManager2 *_ogreCompMgr,
                                     uint8_t _msaa)
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::RetrieveCubePassSceneDefs(
...
//////////////////////////////////////////////////
std::string OgreNextWideAngleCamera::WorkspaceDefinitionName(
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateWorkspaceDefinition(bool _withMsaa)
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateFacesWorkspaces(bool _withMsaa)
{
  ...
  for (uint32_t i = 0u; i < kWideAngleNumCubemapFaces; ++i)
  {
    ...
    for (RenderPassPtr &pass : this->dataPtr->renderPasses)
    {
      OgreNextRenderPass *ogre_nextRenderPass =
        dynamic_cast<OgreNextRenderPass *>(pass.get());
      ogre_nextRenderPass->WorkspaceAdded(
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::DestroyFacesWorkspaces()
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::UpdateRenderPasses()
{
  ...
  for (const auto &pass : this->dataPtr->renderPasses)
  {
    ...
    for (size_t i = 0u; i < kWideAngleNumCubemapFaces; ++i)
    {
      ...
      //
      // Note: We assume ogre_nextRenderPass->OgreCompositorNodeDefinitionName
      // returns the same name (given the same ogre_nextRenderPass ptr)
      GZ_ASSERT(node,
                "This can't be possible. "
                "RemoveRenderPass() should've destroyed the workspace.");

      if (node->getEnabled() != ogre_nextRenderPass->IsEnabled())
      {
  ...
  const uint8_t msaa =
    OgreNextRenderTarget::TargetFSAA(static_cast<uint8_t>(this->antiAliasing));
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::CreateWideAngleTexture()
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::Render()
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::Copy(Image &_image) const
{
  ...
  Ogre::PixelFormatGpu dstOgrePf =
      OgreNextConversions::Convert(_image.Format());
...
//////////////////////////////////////////////////
math::Vector3d OgreNextWideAngleCamera::Project3d(const math::Vector3d &_pt)
    const
{
  ...
  // const Vector4 basePos = viewProj *
  //     Vector4(OgreNextConversions::Convert(_pt));
  const Vector3 basePos = OgreNextConversions::Convert(_pt);
...
//////////////////////////////////////////////////
Ogre::Ray OgreNextWideAngleCamera::CameraToViewportRay(
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::PostRender()
{
  ...
  else
  {
    ...
    Ogre::PixelFormatGpu dstOgrePf = OgreNextConversions::Convert(format);
    Ogre::TextureBox dstBox(
      texture->getInternalWidth(), texture->getInternalHeight(),
      texture->getDepth(), texture->getNumSlices(),
      static_cast<uint32_t>(
...
//////////////////////////////////////////////////
common::ConnectionPtr OgreNextWideAngleCamera::ConnectNewWideAngleFrame(
...
//////////////////////////////////////////////////
RenderTargetPtr OgreNextWideAngleCamera::RenderTarget() const
{
  return this->dataPtr->wideAngleTexture;
}

//////////////////////////////////////////////////
void OgreNextWideAngleCamera::PrepareForCubemapFacePass(
  Ogre::CompositorPass *_pass)
{
  ...
  _pass->getRenderPassDesc()->setClearColour(
    OgreNextConversions::Convert(bgColor));
}

//////////////////////////////////////////////////
void OgreNextWideAngleCamera::PrepareForFinalPass(Ogre::Pass *_pass)
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCameraWorkspaceListenerPrivate::passPreExecute(
  Ogre::CompositorPass *_pass)
{
...
//////////////////////////////////////////////////
void OgreNextWideAngleCamera::SetBackgroundMaterial(MaterialPtr _material)
{
...
//////////////////////////////////////////////////
MaterialPtr OgreNextWideAngleCamera::BackgroundMaterial() const
{
  return this->dataPtr->backgroundMaterial;
}

//////////////////////////////////////////////////
void OgreNextWideAngleCamera::UpdateBackgroundMaterial()
{
