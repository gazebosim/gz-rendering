...
class ProRenderEngineObject : public BaseObject {
protected:
  ProRenderEngineObject() {}

public:
  virtual ~ProRenderEngineObject() {}
...
public:
  virtual ScenePtr Scene() const {
    ...
    // ignition::rendering::v5::ProRenderEngineObject*)this)->
    // ignition::rendering::v5::ProRenderEngineObject::scene’
    // from ‘shared_ptr<ignition::rendering::v5::ProRenderEngineScene>’ to
    // ‘shared_ptr<ignition::rendering::v5::Scene>’ return this->scene;
    return nullptr;
  }

protected:
  ProRenderEngineScenePtr scene;

private:
  friend class ProRenderEngineScene;
};

class ProRenderEngineMaterial : public BaseMaterial<ProRenderEngineObject> {
protected:
  ProRenderEngineMaterial() {}

public:
  virtual ~ProRenderEngineMaterial() {}
...
protected:
  ProRenderEngineRenderTarget() : hostData(0) {}

public:
  virtual ~ProRenderEngineRenderTarget() {}
...
private:
  friend class ProRenderEngineCamera;
};

class ProRenderEngineNode : public BaseNode<ProRenderEngineObject> {
protected:
  ProRenderEngineNode() {}

public:
  virtual ~ProRenderEngineNode() {}
...
protected:
  virtual void SetParent(ProRenderEngineNodePtr _parent) {}
...
class ProRenderEngineSensor : public BaseSensor<ProRenderEngineNode> {
protected:
  ProRenderEngineSensor() {}

public:
  virtual ~ProRenderEngineSensor() {}
};

class ProRenderEngineCamera : public BaseCamera<ProRenderEngineSensor> {
protected:
  ProRenderEngineCamera() {
    this->renderTarget =
        ProRenderEngineRenderTargetPtr(new ProRenderEngineRenderTarget);
    this->renderTarget->SetFormat(PF_R8G8B8);
  };

public:
  virtual ~ProRenderEngineCamera(){};
...
protected:
  ProRenderEngineRenderTargetPtr renderTarget;

private:
  friend class ProRenderEngineScene;
};

class ProRenderEngineScene : public BaseScene {
protected:
  ProRenderEngineScene(unsigned int _id, const std::string &_name)
      : BaseScene(_id, _name) {
    this->id_ = _id;
    this->name_ = _name;
  }
...
protected:
  void CreateStores() {
    ...
    this->sensors =
        ProRenderEngineSensorStorePtr(new ProRenderEngineSensorStore);
    // this->visuals = OptixVisualStorePtr(new OptixVisualStore);
    this->materials =
        ProRenderEngineMaterialMapPtr(new ProRenderEngineMaterialMap);
  }

protected:
  bool InitObject(ProRenderEngineObjectPtr _object, unsigned int _id,
                  const std::string &_name) {
...
private:
  ProRenderEngineScenePtr SharedThis() {
...
protected:
  ProRenderEngineMaterialMapPtr materials;

protected:
  ProRenderEngineSensorStorePtr sensors;

private:
  friend class ProRenderEngineRenderEngine;
};
...
protected:
  virtual ScenePtr CreateSceneImpl(unsigned int _id,
    ...
    int result =
        rprCreateContext(RPR_API_VERSION, plugins, pluginCount,
                         RPR_CREATION_FLAGS_ENABLE_GPU0, NULL, NULL, &context);
    ...
    rprContextSetParameterByKey1u(context, RPR_CONTEXT_RENDER_MODE,
    ...
    rpr_int num_face_vertices[] = {3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3};

    rpr_shape cube = nullptr;
    rprContextCreateMesh(
        context, (rpr_float const *)&cube_data[0], 24, sizeof(vertex),
        (rpr_float const *)((char *)&cube_data[0] + sizeof(rpr_float) * 3), 24,
        sizeof(vertex),
        (rpr_float const *)((char *)&cube_data[0] + sizeof(rpr_float) * 6), 24,
        sizeof(vertex), (rpr_int const *)indices, sizeof(rpr_int),
        (rpr_int const *)indices, sizeof(rpr_int), (rpr_int const *)indices,
        sizeof(rpr_int), num_face_vertices, 12, &cube);
    rprSceneAttachShape(scenePR, cube);
    RadeonProRender::matrix m =
        RadeonProRender::translation(RadeonProRender::float3(-2, 1, 0));
    ...
    rprContextResolveFrameBuffer(context, frame_buffer, frame_buffer_resolved,
                                 true);
...
private:
  friend class common::SingletonT<ProRenderEngineRenderEngine>;

private:
  ProRenderEngineSceneStorePtr scenes;
};

/// \brief Plugin for loading the HelloWorld render engine.
class ProRenderEnginePlugin : public RenderEnginePlugin {
