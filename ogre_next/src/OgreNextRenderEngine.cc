...
#include "gz/rendering/ogre_next/OgreNextNativeWindow.hh"
...
#include "OgreNextGzHlmsPbsPrivate.hh"
#include "OgreNextGzHlmsTerraPrivate.hh"
#include "OgreNextGzHlmsUnlitPrivate.hh"
...
#endif
#include "OgreNextGzHlmsSphericalClipMinDistance.hh"
...
class GZ_RENDERING_OGRE_NEXT_HIDDEN
    gz::rendering::OgreNextRenderEnginePrivate
{
#if HAVE_GLX
  ...
  public: gz::rendering::OgreNextGzHlmsSphericalClipMinDistance
  ...
  public: Ogre::OgreNextGzHlmsPbs *gzHlmsPbs{nullptr};

  /// \brief Custom Unlit modifications
  public: Ogre::OgreNextGzHlmsUnlit *gzHlmsUnlit{nullptr};

  /// \brief Custom Terra modifications
  public: Ogre::OgreNextGzHlmsTerra *gzHlmsTerra{nullptr};
...
//////////////////////////////////////////////////
OgreNextRenderEnginePlugin::OgreNextRenderEnginePlugin()
{
}

//////////////////////////////////////////////////
std::string OgreNextRenderEnginePlugin::Name() const
{
  return OgreNextRenderEngine::Instance()->Name();
}

//////////////////////////////////////////////////
RenderEngine *OgreNextRenderEnginePlugin::Engine() const
{
  return OgreNextRenderEngine::Instance();
}

//////////////////////////////////////////////////
OgreNextRenderEngine::OgreNextRenderEngine() :
  dataPtr(new OgreNextRenderEnginePrivate)
{
  this->dummyDisplay = nullptr;
  this->dummyContext = 0;
  this->dummyWindowId = 0;

  std::string ogrePath = std::string(OGRE_NEXT_RESOURCE_PATH);
  std::vector<std::string> paths = common::Split(ogrePath,
      common::SystemPaths::Delimiter());
  for (const auto &path : paths)
    this->ogrePaths.push_back(path);

  const char *env = std::getenv("OGRE_NEXT_RESOURCE_PATH");
  if (env)
  {
...
//////////////////////////////////////////////////
Ogre::Window * OgreNextRenderEngine::OgreWindow() const
{
  return this->window;
}
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::Destroy()
{
...
//////////////////////////////////////////////////
bool OgreNextRenderEngine::IsEnabled() const
{
  return BaseRenderEngine::IsEnabled();
}

//////////////////////////////////////////////////
std::string OgreNextRenderEngine::Name() const
{
#if (USE_OGRE2_DEFINITIONS)
  return "ogre2";
#else
  return "ogre_next";
#endif
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::AddResourcePath(const std::string &_uri)
{
...
//////////////////////////////////////////////////
Ogre::Root *OgreNextRenderEngine::OgreRoot() const
{
  return this->ogreRoot;
}

//////////////////////////////////////////////////
ScenePtr OgreNextRenderEngine::CreateSceneImpl(unsigned int _id,
    const std::string &_name)
{
  OgreNextScenePtr scene = OgreNextScenePtr(new OgreNextScene(_id, _name));
  this->scenes->Add(scene);
  return scene;
}

//////////////////////////////////////////////////
SceneStorePtr OgreNextRenderEngine::Scenes() const
{
  return this->scenes;
}

//////////////////////////////////////////////////
bool OgreNextRenderEngine::LoadImpl(
...
//////////////////////////////////////////////////
bool OgreNextRenderEngine::InitImpl()
{
  try
  {
    this->InitAttempt();
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::LoadAttempt()
{
  this->CreateLogger();
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::CreateLogger()
{
  ...
  logPath = common::joinPaths(logPath, "ogre_next.log");
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::CreateContext()
{
#ifdef OGRE_BUILD_RENDERSYSTEM_VULKAN
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::CreateRoot()
{
  try
  {
#if (OGRE_NEXT_VERSION_MAJOR > 2)
    this->ogreRoot = new Ogre::Root(nullptr, "", "", "");
#else
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::LoadPlugins()
{
       ...
       iter != this->ogrePaths.end(); ++iter)
  {
         ...
         piter != plugins.end(); ++piter)
    {
      ...
      if (!common::exists(filename))
      {
        filename = filename + "." +
#if (OGRE_NEXT_VERSION_MAJOR > 2)
            std::to_string(OGRE_NEXT_VERSION_MAJOR) + '.' +
            std::to_string(OGRE_NEXT_VERSION_MINOR) + '.' +
            std::to_string(OGRE_NEXT_VERSION_PATCH);
#else
            std::string(OGRE_NEXT_VERSION);
#endif
        if (!common::exists(filename))
        {
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::CreateRenderSystem()
{
...
void OgreNextRenderEngine::RegisterHlms()
{
  ...
  // install path
  std::string mediaPath = common::joinPaths(resourcePath, "ogre_next", "media");
  if (!common::exists(mediaPath))
  {
    // src path
    mediaPath = common::joinPaths(resourcePath, "ogre_next", "src", "media");
  }
  ...
  // The following code is taken from the registerHlms() function in ogre_next
  // samples framework
  ...
  {
    Ogre::OgreNextGzHlmsUnlit *hlmsUnlit = 0;
    // Create & Register HlmsUnlit
    // Get the path to all the subdirectories used by HlmsUnlit
    Ogre::OgreNextGzHlmsUnlit::getDefaultPaths(mainFolderPath,
                                             libraryFoldersPaths);
    ...
    hlmsUnlit = OGRE_NEW Ogre::OgreNextGzHlmsUnlit(
  ...
  {
    Ogre::OgreNextGzHlmsPbs *hlmsPbs = 0;
    // Create & Register HlmsPbs
    // Do the same for HlmsPbs:
    Ogre::OgreNextGzHlmsPbs::GetDefaultPaths(
    ...
    // Create and register
    hlmsPbs =
      OGRE_NEW Ogre::OgreNextGzHlmsPbs(archivePbs, &archivePbsLibraryFolders,
                                     &this->dataPtr->sphericalClipMinDistance,
  ...
  {
    Ogre::OgreNextGzHlmsTerra *hlmsTerra = 0;
    // Create & Register HlmsPbs
    // Do the same for HlmsPbs:
    Ogre::OgreNextGzHlmsTerra::GetDefaultPaths(mainFolderPath,
                                             libraryFoldersPaths);
    ...
    // Create and register
    hlmsTerra = OGRE_NEW Ogre::OgreNextGzHlmsTerra(
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::CreateResources()
{
  const char *env = std::getenv("GZ_RENDERING_RESOURCE_PATH");
...
//////////////////////////////////////////////////
void OgreNextRenderEngine::CreateRenderWindow()
{
  // create dummy window
  std::string handle;
...
//////////////////////////////////////////////////
std::string OgreNextRenderEngine::CreateRenderWindow(const std::string &_handle,
  ...
  if (this->useCurrentGLContext)
  {
    ...
    else
    {
      gzerr << "useCurrentGLContext option specified, "
            ...
            << "the ogre_next rendering requires that also winID is specified."
            << std::endl;
      return std::string();
    }
#endif
...
//////////////////////////////////////////////////
GraphicsAPI OgreNextRenderEngine::GraphicsAPI() const
{
  return this->dataPtr->graphicsAPI;
}

//////////////////////////////////////////////////
NativeWindowPtr OgreNextRenderEngine::CreateNativeWindow(
  ...
  const double _ratio)
{
  ...
  return NativeWindowPtr(new OgreNextNativeWindow(ogreWindow));
}

//////////////////////////////////////////////////
void OgreNextRenderEngine::InitAttempt()
{
  ...
  this->scenes = OgreNextSceneStorePtr(new OgreNextSceneStore);
}

/////////////////////////////////////////////////
std::vector<unsigned int> OgreNextRenderEngine::FSAALevels() const
{
  return this->dataPtr->fsaaLevels;
}

/////////////////////////////////////////////////
OgreNextGzHlmsSphericalClipMinDistance& OgreNextRenderEngine::
SphericalClipMinDistance()
{
...
/////////////////////////////////////////////////
void OgreNextRenderEngine::SetGzOgreRenderingMode(
...
/////////////////////////////////////////////////
Ogre::HlmsPbsTerraShadows *OgreNextRenderEngine::HlmsPbsTerraShadows() const
{
...
Ogre::CompositorWorkspaceListener *
    OgreNextRenderEngine::TerraWorkspaceListener() const
{
  return this->dataPtr->terraWorkspaceListener.get();
}

//////////////////////////////////////////////////
OgreNextRenderEngine *OgreNextRenderEngine::Instance()
{
  return gz::common::SingletonT<OgreNextRenderEngine>::Instance();
}

// Register this plugin
GZ_ADD_PLUGIN(rendering::OgreNextRenderEnginePlugin,
              rendering::RenderEnginePlugin)
GZ_ADD_PLUGIN_ALIAS(rendering::OgreNextRenderEnginePlugin,
                    "gz::rendering::ogre_next::Plugin")
#if (OGRE_NEXT_VERSION_MAJOR <= 2)
GZ_ADD_PLUGIN_ALIAS(rendering::OgreNextRenderEnginePlugin,
