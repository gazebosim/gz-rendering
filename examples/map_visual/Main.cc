...
//////////////////////////////////////////////////
MapVisualPtr createMap(ScenePtr _scene)
{
  // create map visual
  MapVisualPtr map = _scene->CreateMapVisual();

  VisualPtr root = _scene->RootVisual();
  root->AddChild(map);

  map->SetWorldPosition(testPose.Pos());
  map->SetWorldRotation(testPose.Rot());
  ...
  for (auto engineName : engineNames)
  {
    ...
    try
    {
      ...
      if (camera)
      {
        ...
        nodes.push_back(createMap(camera->Scene()));
      }
    }
    catch (...)
    {
