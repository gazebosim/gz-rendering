...
/// \brief Private data class for OgreNextRayQuery
class gz::rendering::OgreNextRayQueryPrivate
{
  /// \brief Ogre ray scene query object for computing intersection.
  public: Ogre::RaySceneQuery *rayQuery = nullptr;

  //// \brief Pointer to camera
  public: OgreNextCameraPtr camera{nullptr};
...
//////////////////////////////////////////////////
OgreNextRayQuery::OgreNextRayQuery()
    : dataPtr(new OgreNextRayQueryPrivate)
{
...
OgreNextRayQuery::~OgreNextRayQuery()
{
  ...
  {
    OgreNextScenePtr ogreScene =
        std::dynamic_pointer_cast<OgreNextScene>(this->Scene());
    if (ogreScene)
    {
...
//////////////////////////////////////////////////
void OgreNextRayQuery::SetFromCamera(const CameraPtr &_camera,
    const math::Vector2d &_coord)
{
  ...
  OgreNextCameraPtr camera = std::dynamic_pointer_cast<OgreNextCamera>(_camera);
  if (camera)
  {
    this->dataPtr->camera = camera;
  }

  OgreNextObjectInterfacePtr ogre_nextObjectInterface =
      std::dynamic_pointer_cast<OgreNextObjectInterface>(_camera);
  if (!ogre_nextObjectInterface)
  {
  ...
  Ogre::Ray ray =
      ogre_nextObjectInterface->OgreCamera()->getCameraToViewportRay(
          screenPos.X(), screenPos.Y());

  auto originMath = OgreNextConversions::Convert(ray.getOrigin());
  ...
  auto directionMath = OgreNextConversions::Convert(ray.getDirection());
...
//////////////////////////////////////////////////
void OgreNextRayQuery::SetFromCamera(const WideAngleCameraPtr &_camera,
                                  uint32_t _faceIdx,
                                  const math::Vector2d &_coord)
{
  ...
  OgreNextWideAngleCameraPtr camera =
    std::dynamic_pointer_cast<OgreNextWideAngleCamera>(_camera);
...
//////////////////////////////////////////////////
void OgreNextRayQuery::SetPreferGpu(bool _preferGpu)
{
  this->dataPtr->preferGpu = _preferGpu;
}

//////////////////////////////////////////////////
bool OgreNextRayQuery::UsesGpu() const
{
...
//////////////////////////////////////////////////
RayQueryResult OgreNextRayQuery::ClosestPoint(bool _forceSceneUpdate)
{
...
//////////////////////////////////////////////////
RayQueryResult OgreNextRayQuery::ClosestPointBySelectionBuffer()
{
...
class GZ_RENDERING_OGRE_NEXT_HIDDEN ThreadedTriRay final
  : public Ogre::UniformScalableTask
{
  /// \brief Stores the result OgreNextRayQueryPrivate::rayQuery->execute
      ...
      for (unsigned int j = 0; j < mesh->SubMeshCount(); ++j)
      {
        ...
        for (unsigned int k = indexStart; k < indexEnd; k += 3)
        {
          ...
          Ogre::Vector3 worldVertexA =
            transform * OgreNextConversions::Convert(vertexA);
          Ogre::Vector3 worldVertexB =
            transform * OgreNextConversions::Convert(vertexB);
          Ogre::Vector3 worldVertexC =
            transform * OgreNextConversions::Convert(vertexC);
#else
          Ogre::Vector3 worldVertexA, worldVertexB, worldVertexC;

          if (bIsAffine)
          {
            worldVertexA = OgreNextConversions::Convert(vertices[indices[k]]);
            worldVertexB =
                OgreNextConversions::Convert(vertices[indices[k + 1]]);
            worldVertexC =
                OgreNextConversions::Convert(vertices[indices[k + 2]]);
          }
          else
          {
            worldVertexA =
              transform * OgreNextConversions::Convert(vertices[indices[k]]);
            worldVertexB =
              transform *
                  OgreNextConversions::Convert(vertices[indices[k + 1]]);
            worldVertexC =
        ...
        if (bestHit.first && distance > bestHit.second)
        {
          ...
          if (bIsAffine)
          {
            result.point = OgreNextConversions::Convert(
              transform * mouseRay.getPoint(distance));
          }
          else
          {
            result.point =
              OgreNextConversions::Convert(mouseRay.getPoint(distance));
          }
...
//////////////////////////////////////////////////
RayQueryResult OgreNextRayQuery::ClosestPointByIntersection(
    bool _forceSceneUpdate)
{
  ...
  const Ogre::Vector3 rayOrigin = OgreNextConversions::Convert(this->origin);
  const Ogre::Vector3 rayDir = OgreNextConversions::Convert(this->direction);
