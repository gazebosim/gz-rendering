 ...
 *
 */
#ifndef GZ_RENDERING_OGRE_NEXT_OGRE_NEXTSCENE_HH_
#define GZ_RENDERING_OGRE_NEXT_OGRE_NEXTSCENE_HH_
...
namespace gz
{
  namespace rendering
  {
    ...
    //
    // forward declaration
    class OgreNextScenePrivate;
    //
    /// \brief OgreNext.x implementation of the scene class
    class GZ_RENDERING_OGRE_NEXT_VISIBLE OgreNextScene :
      public BaseScene
    {
      /// \brief Constructor
      /// \param[in] _id Unique scene Id
      /// \param[in] _name Scene name
      protected: OgreNextScene(unsigned int _id, const std::string &_name);

      /// \brief Destructor
      public: virtual ~OgreNextScene();
      ...
      /// \cond PRIVATE
      /// \brief Certain functions like OgreNextCamera::VisualAt would
      ...
      /// OgreNextCamera::VisualAt (via OgreNextSelectionBuffer)
      ...
      /// by heightmaps (Terra). See OgreNextScene::UpdateAllHeightmaps
      /// Can be null
      ...
      /// \brief Helper function to initialize an ogre_next object
      /// \param[in] _object OgreNext object that will be initialized
      /// \param[in] _id Unique Id to assign to the object
      /// \param[in] _name Name to assign to the object
      protected: virtual bool InitObject(OgreNextObjectPtr _object,
                     unsigned int _id, const std::string &_name);

      /// \internal
      /// \brief Iterates through all Heightmaps and calls
      /// OgreNextHeightmap::UpdateForRender on each of them
      ...
      public: const std::vector<std::weak_ptr<OgreNextHeightmap>> &Heightmaps()
          const;
      ...
      private: OgreNextScenePtr SharedThis();

      /// \brief Root visual in the scene
      protected: OgreNextVisualPtr rootVisual;

      /// \brief Mesh factory for generating ogre meshes
      protected: OgreNextMeshFactoryPtr meshFactory;

      /// \brief A list of ogre sensors, e.g. cameras
      protected: OgreNextSensorStorePtr sensors;

      /// \brief A list of ogre visuals
      protected: OgreNextVisualStorePtr visuals;

      /// \brief A list of ogre lights
      protected: OgreNextLightStorePtr lights;

      /// \brief A list of ogre materials
      protected: OgreNextMaterialMapPtr materials;

      /// \brief A list of ogre heightmaps
      protected: std::vector<std::weak_ptr<OgreNextHeightmap>> heightmaps;

      /// \brief Pointer to the ogre scene manager
      protected: Ogre::SceneManager *ogreSceneManager = nullptr;

      /// \brief Pointer to private data class
      private: std::unique_ptr<OgreNextScenePrivate> dataPtr;

      /// \brief Make the render engine our friend
      private: friend class OgreNextRenderEngine;

      private: friend class OgreNextSceneExt;
    };

    /// \brief OgreNext implementation of the scene extension API
    class OgreNextSceneExt : public SceneExt
    {
      /// \brief Constructor
      /// \param[in] _scene Pointer to scene
      public: OgreNextSceneExt(Scene *_scene);
