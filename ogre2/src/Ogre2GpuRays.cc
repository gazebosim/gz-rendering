  ...
  Ogre::CompositorPass *_pass)
{
  ...
  {
    IGN_ASSERT(dynamic_cast<Ogre::CompositorPassQuad *>(_pass),
...
void Ogre2GpuRays::Setup1stPass()
{
  ...
  if (!wsDef)
  {
    ignerr << "Unable to find workspace definition [" << wsDefName << "] "
           << " for " << this->Name();
  }
...
void Ogre2GpuRays::Setup2ndPass()
{
  ...
  channels.push_back(this->dataPtr->secondPassTexture);
  channels.push_back(this->dataPtr->cubeUVTexture);
  for (size_t i = 0u; i < 6u; ++i)
  {
    if(this->dataPtr->firstPassTextures[i])
    {
      channels.push_back(this->dataPtr->firstPassTextures[i]);
    }
    else
    {
